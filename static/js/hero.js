function renderMethodologyText(){
  ...
  el.innerHTML = `
    ...
    <b>CAGR:</b> log-scaled, 100%&rarr;50pts, ${fmtInt(c.cagr_max_threshold)}%+&rarr;100pts.
...
function renderHero(){
  ...
  if(ORDER.length === 0){
    ...
    document.getElementById('hero-badges').innerHTML = '<span class="badge">FUTURES</span><span class="badge">NO DATA YET</span>';
    return;
  }
  ...
  const liqFreeCounts = ORDER.filter(k => DATA[k].liq_rate === 0).length;
